Solution notes:
I'm trying to fit in "1 day to develop" solution and I choose not optimal, but less code aproaches.

Main problem here is what we are trying to implement in one service close-to-prod solution which always is not 1 service.
Because of that I allow myself (may be wrongly) a few simplifications:
...
If it's not the case, all income requests should be redirected, but this is not optimal (because you wasting resources of 2 instances of services on that for one request).

Solution, happy case, in few words is:
-2. Get request
-1. ssl termination, auth.
0. route to right node which is responsible for this account(accout range)
1. Get request <-- implementation starting from here.
2. validate request
3. block reqiered amout on donor account
4.
  ...
  4.2 a) create request to otside submission service.
      b) track response
	  c) commit blocked amount in first node or release it if failed.
...
2. mark transaction as comlete stage1 + add qty to account2  (state still IN_PROGRESS)
3. mark transaction as comlete stage2 + release the blocked amount or submit it. (after, state = COMPLETE)
...
Notice 3: Solution with rest or mq will provide much more perfomanse, but with more edge cases and gray areas because they are not transactioanal.
...
Notice 5: If one node is down, we will continue to work with subset of accounts. In this case better to have few hot-spot instances to redice downtime.